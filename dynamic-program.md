...
背包问题（Knapsack problem）是一种组合优化的问题，问题可以描述为：给定一些物品，每个物品有自己的重量和价格，问在一个承重限定的背包里，如何选择，能使背包里的总价值最高。

> 每次看不同背包问题的解释，对其中的字母表示很混乱，对此总结其实主要是
>
> - 重量（Weight)
> - 容量/体积（Volume）
> - 容量（Capacity）
...
其实对于背包的承重对应物品的重量，背包的体积对应物品的体积，为了防止代表字母重复，我们按照一种字母表示统一一下。即有 N 个物品放入体积为 V 的背包，放入第 i 个物品的占用的容量为 $C_i$，得到的价值为 $P_i$，问怎么放能得到的最大价值 W，统一按照容量 C、体积 V、价格、价值的字母表达含义。

最基础的背包问题是每种物品只有一件，可以选择放与不放，用 F(i, v) 来表示前 i 件物品放入容量为 v 的背包可以获得的最大价值，则：
$$
W(i, v) = max(W(i-1, v), W(i - 1, v - C_i) + P_i)
$$
